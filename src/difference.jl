...
function -(a::PartitionalClustering, b::PartitionalClustering)
    ...
    ayi = OrderedDict{Int,Int}(k => k for k = 1:size(a.Y, 1))
    byi = OrderedDict{Int,Int}(k => k for k = 1:size(b.Y, 1))
    Y = MatrixDifference(diff(a.Y, b.Y, ayi, a.j, byi, b.j)...)
    amj = OrderedDict{Int,Int}(k => k for k = 1:size(a.M, 2))
    bmj = OrderedDict{Int,Int}(k => k for k = 1:size(b.M, 2))
