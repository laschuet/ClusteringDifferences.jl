...
function -(a::PartitionalClustering, b::PartitionalClustering)
    ...
    i = MatrixDifference(diff(collect(keys(a.i)), collect(keys(b.i)))...)
    j = MatrixDifference(diff(collect(keys(a.j)), collect(keys(b.j)))...)
    C = MatrixDifference(diff(a.C, b.C, a.j, a.j, b.j, b.j))
    W = MatrixDifference(diff(a.W, b.W, a.j, a.j, b.j, b.j))
    Y = MatrixDifference(diff(a.Y, b.Y, Dict(k => k for k = 1:size(a.Y, 1)), a.j, Dict(k => k for k = 1:size(b.Y, 1)), b.j))
    M = MatrixDifference(diff(a.M, b.M, a.i, Dict(k => k for k = 1:size(a.M, 2)), b.i, Dict(k => k for k = 1:size(b.M, 2))))
