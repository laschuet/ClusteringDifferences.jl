...
function -(a::PartitionalClustering, b::PartitionalClustering)
    ...
    Y = MatrixDifference(diff(a.Y, b.Y, OrderedDict(k => k for k = 1:size(a.Y, 1)), a.j, OrderedDict(k => k for k = 1:size(b.Y, 1)), b.j)...)
    M = MatrixDifference(diff(a.M, b.M, a.i, OrderedDict(k => k for k = 1:size(a.M, 2)), b.i, OrderedDict(k => k for k = 1:size(b.M, 2)))...)
